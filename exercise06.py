...
import random
class cannon_shell:
    def __init__(self,initial_velocity=2,g=0.0098,range=0,height=0,target_range=10,target_height=1,time_step=0.005,\
                 da=0.00001*math.pi,\
    ...
    def run(self):   #绘制有随机误差时的炮线,并计算误差的平方均值
        ...
        while t<=200:
            ...
            while (yi[-1] >= 0):
                xi.append(xi[-1] + self.dt * vx)
                yi.append(yi[-1] + self.dt * vy)
                v = ((vx + self.Vw * (1 + random.uniform(-1,1) * 0.1)) ** 2 + vy ** 2) ** 0.5
                vx = vx \
                     - math.exp(-yi[-1] / 10) * self.b_m * v * vx * self.dt
                vy = vy \
                     - self.g * self.dt \
                ...
                if (abs(xi[-1] - self.xt) < 2 and abs(yi[-1] - self.yt) < 2):  #取较靠近目标的点
                    dxdy.append(n)
            t+=1
